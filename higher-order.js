...
let evenNumbers = mixedNumbers.filter((num,i,arr) => arr[i] % 2 === 0);
...
let postTaxPrices = prices.map((num,i,arr) => {
    var postTaxPrice = [];
    postTaxPrice.push(num * 1.07);
    return postTaxPrice;
} );
...
let totalPopulation = populations.reduce((runningTotal,num,i,arr) => runningTotal + num);
...
let myStrongest = monstersInYourPocket.filter((num,i,arr) => num.CP > 200)// Your code here
...
let ordersTotal = orders.map((num,i,arr) => num = num.price * (1 + num.tax));// Your code here
...
let bobsTotal = purchases.filter((num,i,arr) => num.owner === 'Bob').map((num,i,arr) => num = num.price).reduce((rt,num,i,arr) => rt + num);// Your code here
